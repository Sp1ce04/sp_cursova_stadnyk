...
unsigned int GetTokens(FILE* F, Token TokenTable[], FILE* errFile)
{
	...
	while (1)
	{
		...
		case Letter:
		{
			...
			else if (!strcmp(buf, "Body"))		temp_type = StartProgram;
			else if (!strcmp(buf, "Data"))		temp_type = Variable;
			else if (!strcmp(buf, "Integer_4"))	temp_type = Type;
			else if (!strcmp(buf, "Get"))		temp_type = Input;
			else if (!strcmp(buf, "Put"))	temp_type = Output;
